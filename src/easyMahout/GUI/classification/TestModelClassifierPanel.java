...
public class TestModelClassifierPanel extends JPanel {
	...
	private final static Logger log = Logger
			.getLogger(DataModelClassifierPanel.class);

	public TestModelClassifierPanel() {
		super();
		setBorder(new TitledBorder(new LineBorder(new Color(0, 0, 0), 1, true),
				"Test model", TitledBorder.CENTER, TitledBorder.TOP, null, null));
		...
		textInputPath.getDocument().addDocumentListener(
			...
			public boolean verify(JComponent input) {
				...
				try {
					...
					if (!text.equals("")) {
						...
						MainGUI.writeResult("You have to choose an input!",
								Constants.Log.ERROR);
		...
		textModelPath.getDocument().addDocumentListener(
				new TextFieldChangeListener());
			...
			public boolean verify(JComponent input) {
				...
				try {
					...
					if (!text.equals("")) {
						...
						log.error(text + " is empty");
						MainGUI.writeResult(
								"You have to choose an model input!",
								Constants.Log.ERROR);
					...
					log.error(text + " is empty");
					MainGUI.writeResult("You have to choose an model input!",
							Constants.Log.ERROR);
					textModelPath.setBackground(new Color(240, 128, 128));
					return false;
				}
		...
		textLabelIndexPath.getDocument().addDocumentListener(
				new TextFieldChangeListener());
			...
			public boolean verify(JComponent input) {
				...
				try {
					...
					if (!text.equals("")) {
						...
						log.error(text + " is empty");
						MainGUI.writeResult(
								"You have to choose a label index input!",
								Constants.Log.ERROR);
						textLabelIndexPath.setBackground(new Color(240, 128,
								128));
						return false;
					}
				} catch (NumberFormatException e) {
					log.error(text + " is empty");
					MainGUI.writeResult(
							"You have to choose label index input!",
							Constants.Log.ERROR);
		...
		add(textOutputPath);
		textOutputPath.getDocument().addDocumentListener(
				new TextFieldChangeListener());
			...
			public boolean verify(JComponent input) {
				...
				try {
					...
					if (!text.equals("")) {
						...
						log.error(text + " is empty");
						MainGUI.writeResult("You have to choose an output!",
								Constants.Log.ERROR);
			...
			public void actionPerformed(ActionEvent e) {
				if (MainGUI.isDistributed()) {
					...
					int returnVal = chooser
							.showOpenDialog(TestModelClassifierPanel.this);
					if (returnVal == FolderChooser.APPROVE_OPTION) {
						...
						MainGUI.writeResult(
								"Error searching the input directory.",
								Constants.Log.ERROR);
					...
					if (i == JFileChooser.APPROVE_OPTION) {
						...
						MainGUI.writeResult("Error opening the file.",
								Constants.Log.ERROR);
			...
			public void actionPerformed(ActionEvent e) {
				if (MainGUI.isDistributed()) {
					...
					if (i == JFileChooser.APPROVE_OPTION) {
						...
						MainGUI.writeResult("Error searching the file.",
								Constants.Log.ERROR);
		...
		btnCreate.addActionListener(new ActionListener() {
			public void actionPerformed(ActionEvent e) {
				...
				if (!filePath.isEmpty() && !output.isEmpty()) {
					try {
						MainGUI.writeResult("Starting to test the classifier",
								Constants.Log.INFO);
						ClassifierBuilder.testClassifier();
					} catch (Exception e1) {
						e1.printStackTrace();
						// MainGUI.writeResult("Not able to build the classifier, GENERIC",
						// Constants.Log.ERROR);
					}
				} else
					MainGUI.writeResult(
							"You have to specify both input, model, label index and output source file!",
							Constants.Log.ERROR);
		...
		btnHelp = new JButton(new ImageIcon(
				TestModelClassifierPanel.class
		...
		// Help Tip
		helpTooltip = new HelpTooltip(btnHelp,
				ClassifierTips.CLASSIFIER_TESTMODEL);
		add(helpTooltip);
